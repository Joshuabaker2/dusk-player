...
* `AccentColorTV` is the tvOS target's global accent (`ASSETCATALOG_COMPILER_GLOBAL_ACCENT_COLOR_NAME`). It is the dark tab bar chrome color, not a brand token: the global accent is the UIKit window tint, and the tvOS tab bar must never inherit coral from it. SwiftUI content on tvOS still gets `AppAccent` from the root `.tint(Color.duskAccent)`.
